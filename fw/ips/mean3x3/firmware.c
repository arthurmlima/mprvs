...
#define GET_LOCAL_PIXEL          (*(volatile uint32_t*)0x0223FF00)
...
uint8_t get_local_pixel(uint32_t x_dest, uint32_t y_dest, uint32_t step, uint32_t frame)
{
// resetar o buff de envio
SET_PIXEL_BUFFER=0ULL;
SET_PIXEL_BUFFER_TOP=0;
uint8_t pixel;
uint32_t pixel_set_bit =0;
pixel_set_bit = 1<<15;


    set_pm_pixel(pixel_set_bit);
    set_pm_x_dest(x_dest);
    set_pm_y_dest(y_dest);
    set_pm_step(step);
    set_pm_frame(frame);
...
{
    int local=0;
        if(x>=X_LOCAL*SUB_IMAGE_WIDTH && x<(X_LOCAL*SUB_IMAGE_WIDTH+SUB_IMAGE_WIDTH) )
        {
           
            if(y>=Y_LOCAL*SUB_IMAGE_HEIGHT && y< (Y_LOCAL*SUB_IMAGE_HEIGHT+SUB_IMAGE_HEIGHT) ) 
             {
            local = 1;
             } 
        }
        if (local)
        {
            return get_local_pixel(x,y,s,f);
        }
        else
        {
